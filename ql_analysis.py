...
def ql(nx, ny, nz, Reynolds):
    # # Load raw data
    ff = h5py.File('slices_ql/slices.h5', 'r')
    gg = h5py.File('dump_ql/dump.h5', 'r')
...
# Compute mean velocity
    ...
    # return print('ql min <u>ty:', min(umeanvty))
...
# Save all arrays for plots
    ...
    np.save('u_xy_nw_ql', u_xy_nw_inst)            # instantaneous slice, second to last slice
    np.save('u_xy_cl_ql', u_xy_cl_inst)
    np.save('u_xy_NW_ql', u_xy_NW_end)                        # NW fluctuations (field - mean)*
    np.save('u_xy_CL_ql', u_xy_CL_end)                            # CL fluctuations (field - mean)*
    np.save('uh_xy_cl_ql', uh_xy_cl)           # high modes instantaneous
    np.save('ul_xy_cl_ql', ul_xy_cl)           # high modes instantaneous
    np.save('uvert_ql', uvert_all)             # xz plane used for mean calculation
    np.save('ek_xy_ql', ek_xy_timeavg2)        # 2d spectra*
    np.save('ek_x_ql', ek_x_timeavg2)          # 1d spectra, x*
    np.save('ek_y_ql', ek_y_timeavg2)          # 1d spectra, y*
    np.save('u_tau_ql', u_tau)                 # u_tau calculation
    np.save('uzmeanty_ql', uzmeanty)           # velocity gradient avg in y and t*
    np.save('uzmeany_ql', uzmeany)             # velocity gradient avg in y (for plots every time step)*
    np.save('uvmeany_ql', uvmeany[t_turb:tslices, :])
    np.save('umeanvty_ql', umeanvty)
    np.save('Upyt_ql', U_pyt)
    np.save('upsq_yt_ql', upsq_yt)
    np.save('upsq_xyt_ql', upsq_xyt)
    np.save('upvert_plot_ql', upvert_plot)
    np.save('f_upvert_plot_ql', f_upvert_plot)
    np.save('vp_ql', vp)
    np.save('wp_ql', wp)
